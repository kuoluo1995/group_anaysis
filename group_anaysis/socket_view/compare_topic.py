...
from services.service import get_compared_topics_by_person_ids
from tools import json_utils


class ComparedTopicsByPersonIds(WebsocketConsumer):
    def connect(self):
        self.accept()
    ...
    def receive(self, text_data):
        ...
        if 'person_ids1[]' in request and request['person_ids1[]'] and 'person_ids2[]' in request and request[
            'person_ids2[]'] and 'populate_ratio' in request and request[
            'populate_ratio'] and 'max_topic' in request and request['max_topic'] and 'min_sentence' in request and \
                request['min_sentence']:
            person_ids1 = request['person_ids1[]']
            person_ids1 = [int(_id) for _id in person_ids1]
            person_ids2 = request['person_ids2[]']
            ...
            try:
                ...
                _name = 'error_compare_topic_' + str(datetime.datetime.now()).replace(' ', '-').replace(':', '_')
                json_utils.save_json(request, _name)
